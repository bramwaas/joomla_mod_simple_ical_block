 ...
 */
    ...
    */
     ...
     *    ['tzid_ui']      if avail create ['tz_ui]', else try Factory::getApplication()->get('offset');, or as last resort 'UTC'. 
    ...
    static function getData($instance)
    {
        ...
        if (empty($instance['tz_ui']))
            try {
                $instance['tzid_ui'] = Factory::getApplication()->get('offset');
                $instance['tz_ui'] = new \DateTimeZone($instance['tzid_ui']);
            } catch (\Exception $exc) {}
        if (empty($instance['tz_ui'])) {
